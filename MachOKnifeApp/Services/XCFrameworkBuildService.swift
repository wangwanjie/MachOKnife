        ...
        process.terminationHandler = { [weak self] process in
            ...
            if process.terminationReason == .exit, process.terminationStatus == 0 {
                ...
                completionHandler(.failure(NSError(
                    ...
                    userInfo: [NSLocalizedDescriptionKey: "XCFramework build cancelled."]
                )))
            } else {
                completionHandler(.failure(NSError(
                    domain: "MachOKnife.XCFrameworkBuild",
                    code: Int(process.terminationStatus),
                    userInfo: [NSLocalizedDescriptionKey: output.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "XCFramework build failed." : output]
                )))
            }
        }
...
def prepare_headers(source_headers_dir: Path, output_headers_dir: Path, *, umbrella_header_name: str | None, module_name: str | None) -> None:
    ...
    for source_path in source_headers_dir.rglob("*"):
        ...
        if source_path.suffix == ".h":
            header_names.add(relative_path.name)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build an XCFramework from a static iOS SDK library with retagged simulator and Mac Catalyst slices.")
